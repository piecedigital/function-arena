...
function gameLoop() {
  var start = Date.now();
  Object.keys(gamepads).map(function (name) {
...
function displayInputs(inputsArray, padInfo) {
  ...
  inputsArray.map(function (input) {
    if (isNaN(parseInt(input))) {
      ...
      var img,
          cssBtn = document.createElement("span");
      cssBtn.className = "css-button";
      switch (parseInt(configBtn)) {
        case 8:
          cssBtn.innerText = "SELECT";break; // select/back
        case 9:
          cssBtn.innerText = "START";break; // start
        case 10:
          cssBtn.innerText = "HOME";break; // home
        default:
          img = getInputImage(configBtn);
      }
...
function makeInputDisplayElements(padInfo, inputs) {
  ...
  if (inputs.depressed) {
    ...
    if (inputs.depressed[4]) {
      ...
      if (inputs.depressed[0] && inputs.depressed[3] && inputs.depressed[5]) {
    ...
    if (inputs.depressed[6]) {
      if (inputs.depressed[1] && inputs.depressed[2] && inputs.depressed[7]) {
  ...
  if (inputs.onePress) {
    ...
    Object.keys(inputs.onePress).map(function (btn) {
  ...
  if (inputs.oneAxis === inputs.axis || inputs.onePress) {
    if (inputs.axis !== "n") {
      var elem = document.createElement("span");
      elem.className = "axis-input";
...
function getCharacters() {
          ...
          moveData: {
            F: {
              norm: {
                ...
                func: function func() {
                  console.log("performing move:", this.displayName);
                }
              },
              ex: {
                displayName: "Shoryuken",
                ...
                func: function func() {
                  console.log("performing move:", "EX", this.displayName);
                }
              },
              sup: {
  ...
  Object.keys(characters).map(function (name) {
...
function populateCharacters() {
  ...
  characters.map(function (charName) {
    var button = document.createElement("button");
    button.type = "button";
    button.name = "button";
    button.innerText = charName.split("_").map(function (str) {
      return str = str[0].toUpperCase() + str.slice(1, 100);
    }).join(" ");

    [p1SideElem, p2SideElem].map(function (elem, side) {
...
function Player(data) {
  ...
  var constructor = (function (data) {
    ...
    var actionVariants = [["F", "SUPER", "1+2+7", 50],
    // ["F", "SUPER", "1", 50],
    // ["F", "SUPER", "2", 50],
    // ["F", "SUPER", "7", 50],
    ["F", "EX", "1+2", 20], ["F", "EX", "2+7", 20], ["F", "EX", "1+7", 20], ["B", "SUPER", "1+2+7", 50],
    // ["B", "SUPER", "1", 50],
    // ["B", "SUPER", "2", 50],
    // ["B", "SUPER", "7", 50],
    ["B", "EX", "1+2", 20], ["B", "EX", "2+7", 20], ["B", "EX", "1+7", 20], ["B", "HK", "7", 20], ["B", "MK", "2", 18], ["B", "LK", "1", 15], ["F", "HK", "7", 20], ["F", "MK", "2", 18], ["F", "LK", "2", 15], ["F", "SUPER", "0+3+5", 50],
    // ["F", "SUPER", miniMap.lp + "", 50],
    // ["F", "SUPER", "3", 50],
    // ["F", "SUPER", "5", 50],
    ["F", "EX", "0+3", 20], ["F", "EX", "3+5", 20], ["F", "EX", "0+5", 20], ["B", "SUPER", "0+3+5", 50],
    // ["B", "SUPER", miniMap.lp + "", 50],
    // ["B", "SUPER", "3", 50],
    // ["B", "SUPER", "5", 50],
    ["B", "EX", "0+3", 20], ["B", "EX", "3+5", 20], ["B", "EX", "0+5", 20], ["B", "HP", "5", 20], ["B", "MP", "3", 18], ["B", "LP", "0", 15], ["F", "HP", "5", 20], ["F", "MP", "3", 18], ["F", "LP", "0", 15]];

    // add inputs
    // HCB & HCF
    // !!! null !!!
    // DQCB & DQCF
    actionVariants.map(function (_ref) {
      var _ref2 = _slicedToArray(_ref, 4);

      var d = _ref2[0];
      var btn = _ref2[1];
      var btnNum = _ref2[2];
      var recovery = _ref2[3];

      var pattern = "DQC";
      var input = d === "F" ? ["d", "df", "f", "d", "df", "f"] : ["d", "db", "b", "d", "db", "b"];
      _this.actionsArray.push({
        pattern: pattern,
        dir: d,
        btn: btn,
        btnNum: btnNum,
    ...
    // HCB & HCF
    actionVariants.map(function (_ref3) {
      var _ref32 = _slicedToArray(_ref3, 4);

      var d = _ref32[0];
      var btn = _ref32[1];
      var btnNum = _ref32[2];
      var recovery = _ref32[3];
    ...
    // ZB & ZF
    actionVariants.map(function (_ref4) {
      var _ref42 = _slicedToArray(_ref4, 4);

      var d = _ref42[0];
      var btn = _ref42[1];
      var btnNum = _ref42[2];
      var recovery = _ref42[3];
    ...
    // QCB & QCF
    actionVariants.map(function (_ref5) {
      var _ref52 = _slicedToArray(_ref5, 4);

      var d = _ref52[0];
      var btn = _ref52[1];
      var btnNum = _ref52[2];
      var recovery = _ref52[3];
  ...
  var alterDashState = function alterDashState(dir, setTime) {
    var phase = _this2.activeProps.dashState.phase,
        time = _this2.activeProps.dashState.time;
    if (setTime) {
      time = 10;
    } else {
      time--;
    ...
    if (dir === "N") {
      dir = _this2.activeProps.dashState.dir;
    }

    if (dir) {
      switch (_this2.activeProps.dashState.phase) {
        case "prep":
          phase = "canDash";break;
        case "canDash":
          phase = null;break;
      ...
      if (time <= 0) {
        phase = null;
        _this2.activeProps.dashState.dir = null;
      } else {
        phase = phase;
      }
    }

    _this2.activeProps.dashState = {
      ...
      phase: phase,
      time: time,
      dir: dir || _this2.activeProps.dashState.dir
    };
  ...
  this.setActionState = function (state) {
    ...
    Object.keys(state).map(function (name) {
      switch (name) {
        case "grounded":
        case "crouched":
          state[name] !== undefined ? _this3.actionState.position[name] = state[name] : null;
          break;
        case "canTakeInput":
        case "recovery":
        case "inputsToPurge":
        case "lastDirection":
        default:
          state[name] !== undefined ? _this3.actionState[name] = state[name] : null;
          break;
      }
    });
  };
  ...
  this.performFrameActions = function (padInfo, inputs) {
    ...
    if (this.actionState.recovery === 1) {
      ...
      if (this.actionState.inputsToPurge) this.actionState.inputsToPurge.map(function (_, ind) {
        var place = padInfo.recordedInputs.indexOf(_this4.actionState.inputsToPurge[ind]);
  ...
  this.captureMove = function (padInfo, axis) {
    ...
    var read = maxRead;
    var whatImWorkingWith = record.slice(read * -1).filter(function (n) {
      return !!n;
    });
    ...
    var performMove = (function (movesCaptured) {
      ...
      var keys = Object.keys(movesCaptured).filter(function (n) {
        return !!movesCaptured[n];
      }); // ["SUPER", "EX", "NORM"]
      ...
      if (keys.length > 0) {
        ...
        keys.map(function (key) {
          ...
          if (_this5.actionState.canTakeInput) doAttack.bind(_this5, action, meter, key)();
        });
      } else {
      ...
      function doAttack(action, meter, type) {
        ...
        this.setActionState({
          ...
          inputsToPurge: whatImWorkingWith.map(function (_, ind) {
            return record[record.length - (whatImWorkingWith.length - ind)];
          })
        });
      ...
      function doMovement(action, direction, stats) {
        ...
        var capitalAction = action.replace(/(.)/, function (_, l) {
    ...
    this.activeActionsArray.map(function (action, actionInd) {
      if (!_this6.actionState.canTakeInput) return;
      ...
      switch (action.btn) {
        case "SUPER":
          occupied = !!movesCaptured.SUPER;alreadyUsed = movesCaptured.SUPER === action.SUPER;break;
        case "EX":
          occupied = !!movesCaptured.EX;alreadyUsed = movesCaptured.EX === action.EX;break;
        default:
          occupied = !!movesCaptured.NORM;alreadyUsed = movesCaptured.NORM === action.NORM;break;
      }
      ...
      if (padInfo.recordedInputs.length >= action.input.length) {
        ...
        for (ind = whatImWorkingWith.length - 1; ind >= 0; ind--) {
          ...
          if (inputs) {
            ...
            var alteredInputs = inputs.map(function (input) {
        ...
        if (whatImatchedIsWhatIwant) {
          ...
          switch (action.btn) {
            case "SUPER":
              movesCaptured.SUPER = action;break;
            case "EX":
              movesCaptured.EX = action;break;
            default:
              movesCaptured.NORM = action;break;
          }
  ...
  this.setPlayerPuppet = function (puppet) {
    ...
    var newActionsArray = [];
    this.actionsArray.map(function (input) {
      var willAdd = true;

      var puppetSpecials = _this7.puppet.specials;
        ...
        keysToCheck.map(function (key) {
          ...
          if (move[key]) {
            ...
            if (realType(move[key]) === "object") {
              var checkFault = function checkFault(data) {
...
function createCube() {
  var _ref6 = arguments.length <= 0 || arguments[0] === undefined ? {
    x: 0,
    y: 0,
    z: -.5,
    color: 0x00ff00
  ...
  return {
    ...
    move: function move(axis) {
      ...
      axisValuesArray.map(function (dir) {
