  ...
  function brfy() {
    setTimeout(() => {
      ...
      if(!(timer-1) && !executing && !failure) {
        ...
        executing = true;
        cp.exec("browserify ./dist/main.js --debug -o ./dist/bundle.js", function (err) {
          if(err) {
            logOut(err, true, {
              type: "error"
            });
        ...
        cp.exec("browserify ./dist/main.js --debug -g [ envify --NODE_ENV 'production' ] | uglifyjs > ./dist/bundle-live.js", function (err) {
          if(err) {
            logOut(err, true, {
              type: "error"
            });
